...
namespace tops
{
  ...
  void GeneralizedHiddenMarkovModel::restore_model(std::string &model_name, const ProbabilisticModelParameters &parameters)
  {
    ...
    if ((cfg.size()) > 0 && (cfg[0] == '['))
    {
      ...
      if (modelreader.load(cfg))
      {
        ...
        if (modelpar != NULL)
        {
          ...
          {
            std::cerr << "[INFO]: submodel  "
                << submodelstr << std::endl;
    ...
    {      
      std::cerr << "\t[INFO] GHMM CALLING init open file creator: "  << cfg << std::endl;
  ...
  void GeneralizedHiddenMarkovModel::posteriorProbabilitiesNoClasses(const Sequence &s, fMatrix &postProbs) const
  {
    ...
    for (int k = 0; k < nstates; k++)
    {
      ...
      alpha(k, 0) += _all_states[k]->observation()->prefix_sum_array_compute(0, 0, _all_states[k]->getInputPhase()) + _all_states[k]->duration_probability(1);
      if (alpha(k, 0) > -HUGE)
      {
  ...
  void GeneralizedHiddenMarkovModel::posteriorProbabilitiesWithClasses(const Sequence &s, SparseMatrixPtr probabilities) const
  {
    ...
    for (int k = 0; k < nstates; k++)
    {
      alpha(k, 0) = getInitialProbabilities()->log_probability_of(k) + _all_states[k]->observation()->prefix_sum_array_compute(0, 0, _all_states[k]->getInputPhase()) + _all_states[k]->duration_probability(1);
      if (alpha(k, 0) > -HUGE)
      {
  ...
  double GeneralizedHiddenMarkovModel::forward(const Sequence &s, Matrix &a) const
  {
    ...
    for (int k = 0; k < nstates; k++)
    {
      if (_all_states[k]->isGeometricDuration())
        ...
        alpha(k, 0) = getInitialProbabilities()->log_probability_of(k) + _all_states[k]->observation()->prefix_sum_array_compute(0, 0, _all_states[k]->getInputPhase());
    ...
    if (_terminal_probabilities != NULL)
      sum = alpha(0, size - 1) + _terminal_probabilities->log_probability_of(0);
    else
      sum = alpha(0, size - 1);
  ...
  for(int i = 0; i < size; i++) {
      ...
      for(int k = 0; k < nstates; k++){
          std::cerr << " "  << getStateName(k) << " " << alpha(k, i) << std::endl;
      }
    ...
    for (int i = 1; i < size; i++)
    {
      for (int k = 0; k < nstates; k++)
      {
        ...
        for (int d = i; d > 0; d--)
        {
          ...
          {
            alpha(k, i) = log_sum(alpha(k, i), alpha(p, i - d) + _all_states[p]->transition()->log_probability_of(k) + _all_states[k]->duration_probability(d) + _all_states[k]->observation()->prefix_sum_array_compute(i - d + 1, i, _all_states[k]->getInputPhase()));
          }
        }
      }
    }
...
#if 0
	std::cerr << getStateName(k) << "->" ;
	std::cerr << getStateName(id) << ":";
	std::cerr << beta(k, i) << ",";
	std::cerr << _all_states[k]->transition()->log_probability_of(id) << ",";
	std::cerr << _all_states[id]->backwardSum(beta,s,i,valid_positions) << std::endl;
#endif
  ...
  for(int i = 0; i < size; i++) {
    std::cerr << "i: " << i << std::endl;
    for(int k = 0; k < nstates; k++){
      std::cerr << " "  << getStateName(k) << " " << beta(k, i) << std::endl;
    }
                                               ...
                                               Matrix &g) const
  {
    ...
    for (int i = 0; i < (int)possible_path.size(); i++)
    {
      ...
      for (int p = 0; p < (int)next_states.size(); p++) // is it necessary to go into the sucessors? perhaps not since they will/were be i sometime
      {
  ...
  std::string GeneralizedHiddenMarkovModel::str() const
  {
    ...
    for (int i = 1; i < nstates; i++)
    {
      if (!close(exp(_initial_probabilities->log_probability_of(i)), 0.0, 1e-10))
        out << ";\n \"" << getStateName(i) << "\": " << exp(_initial_probabilities->log_probability_of(i));
    }
    out << ")" << std::endl;
